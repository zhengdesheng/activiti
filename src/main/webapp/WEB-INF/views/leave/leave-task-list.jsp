		...
		<tbody>
			<c:forEach items="${records }" var="leave">
				<tr>
					...
					<td><c:if test="${empty leave.task.assignee }">
							<a class="btn" href="${ctx }/leave/task/claim?id=${leave.task.id}"><i
								class="icon-eye-open"></i>签收</a>
						</c:if> <c:if test="${not empty leave.task.assignee }">
							<a class="btn" href="${ctx }/leave/task/view?id=${leave.task.id}"><i
								class="icon-user"></i>办理</a>
						</c:if></td>
				</tr>
			</c:forEach>
		</tbody>
